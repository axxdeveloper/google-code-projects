 ...
 * @author Isaac
 */
public class XPointTextAreaNotationLocator implements XWidgetLocator {

    public void locate(XObjectScene objectScene, Point location, Rectangle bounds) {
        objectScene.setXWidgetLocator(null);
        XTextAreaWidgetPO sourceWidgetPO = new XTextAreaWidgetPO();
        sourceWidgetPO.setPreferredLocation(new PointPO(location));
        if ( bounds != null ) {
            sourceWidgetPO.setPreferredBounds(new RectanglePO(bounds));
        }
        XEditableLabelWidgetPO targetWidgetPO = new XEditableLabelWidgetPO();
        targetWidgetPO.setPointer( true );
        targetWidgetPO.setLabel("     ");
